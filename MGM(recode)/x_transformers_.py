...
# from MLM.ssma import SSMA
# from MLM.dealPOI import zoneDict
# constants
...
class TransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        # #特征融合模块
        # self.ssma_s1 = SSMA(1, 1)
        # #POI向量编码模块
        # self.timelinear = nn.Linear(24, emb_dim).cuda()
        # self.mergelinear = nn.Linear(emb_dim, emb_dim).cuda()
        # self.earlylinear  = nn.Linear(emb_dim * 3, emb_dim).cuda()
    ...
    def forward(
        ...
        **kwargs
        ...
        # # 在这里形成 batch length width 的POI向量
        # poiEmbedding=[]
        # traj_array = x.cpu().numpy()
        ...
        # poiEmbedding=torch.tensor(poiEmbedding).cuda()
        # # print('shape of poiEmbedding is: ',poiEmbedding.shape)
        ...
        # # 定义POI目标维度
        # target_size = (x.size()[0], x.size()[1], x.size()[2])

        # # 将输入张量投影为指定形状的输出张量
        # linear_layer = nn.Linear(64, x.size()[2]).cuda()

        # x_otherPOI = linear_layer(poiEmbedding)
        # # print('shape of x_otherPOI is: ', x_otherPOI.shape)
        ...
        # import numpy as np
        # x_other
        # # list_shape = np.array(x_other).shape
        # # print(list_shape)
        # x_otherlist=[]
        # for values in x_other:
        # # 创建独热编码矩阵
        #     one_hot_matrix = np.eye(24)[values[:44]]
        #     x_otherlist.append(one_hot_matrix)

        # x_other = torch.Tensor(x_otherlist).cuda()
        # x_otherTime = self.timelinear(x_other)
        # # # merged_tensor = torch.cat((x, x_otherTime), dim=2)
        # # merged_tensor = x+x_otherTime
        # # # linear_layer = nn.Linear(x.size()[2], x.size()[2]).cuda()
        # # x = self.mergelinear(merged_tensor)
        # # print('shape of x_otherTime is:',x_otherTime.shape)
        # fusion_X = self.ssma_s1(x_otherTime, x_otherPOI)
        # fusion_XY = self.ssma_s1(x, fusion_X)
        # x = fusion_XY

        # poi 在只考虑位置时才注释以下这段代码
        # fusion_XY = self.ssma_s1(x, x_otherPOI)


        #后融合模式代码
        # early = torch.cat((x, x_otherTime, x_otherPOI), dim=2)
        # # lateTensor = self.earlylinear(early)
        # lateTensor=early
        ...
        # early = torch.cat((out, x_otherTime, x_otherPOI), dim=2)
        # lateTensor = early
        return out
